...
from config.ai_setup import configure_ai, SYSTEM_PROMPT
from workflows.assistant_workflow import create_workflow
from agents import (
    space_events,
    astronomy_images,
...
load_dotenv()
llm, search_tool = configure_ai()

# Initialize agents
tools = [
    search_tool,
    space_events.space_events_agent,
    astronomy_images.astronomy_image_agent,
    educational_resources.educational_resources_agent,
    iss_location.iss_location_agent
]

# Create workflow
react_graph_memory = create_workflow(llm, tools)

# Streamlit UI
st.title("🌠CosmoBot🌠: Space Exploration Assistant")
st.caption("Your AI-powered astronomy companion")
...
# Handle user input
if user_input := st.chat_input("Ask about space..."):
    ...
    try:
        ...
        # Handle image URLs
        if image_url := re.search(r'(https?://\S+\.(?:png|jpg|jpeg|gif))', assistant_response):
            st.image(image_url.group(1))
